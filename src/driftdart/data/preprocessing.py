...
def load_and_prepare_dataframe(
    ...
    if has_header:
        df = pd.read_csv(csv_path, low_memory=False, nrows=max_rows, sep=delimiter)
    else:
        df = pd.read_csv(csv_path, low_memory=False, nrows=max_rows, sep=delimiter, header=None)
        if column_names:
